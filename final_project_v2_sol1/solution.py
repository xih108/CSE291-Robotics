...
class Solution(SolutionBase):
    ...
    def act(self, env: FinalEnv, current_timestep: int):
        ...
        if self.phase == 1:
            ...
            target_pose_left = Pose([self.selected_x, 0.5, self.selected_z + 0.65], euler2quat(np.pi, -np.pi / 3, -np.pi / 2))
            self.diff_drive(robot_left, 9, target_pose_left)

            target_pose_right = Pose([self.selected_x, -0.5, self.selected_z + 0.55], euler2quat(np.pi, -np.pi / 3, np.pi / 2))
            ...
            if self.counter == 2000 / 5:
                ...
                p, q = pose.p, pose.q
                p[1] = (self.spade_length + self.spade_width) / 4
        ...
        if self.phase == 3:
            ...
            p[1] -= 1
            p[2] += 5
            q = euler2quat(np.pi, np.pi / 8, -np.pi / 4)
            self.pose_left = Pose(p, q)
            self.counter += 1
            self.diff_drive(robot_left, 9, self.pose_left)
    ...
    def pick_box(self, c):
        ...
        for i in self.box_ids:
            ...
            if len(m[0]):
                ...
                size[i] = abs(diff[0]) / 2
                gmax_x = max(gmax_x, position[i][0])
                gmin_x = min(gmin_x, position[i][0])
                gmax_y = max(gmax_y, position[i][1])
                gmin_y = min(gmin_y, position[i][1])
        ...
        while (low < gmax_x):
            ...
            min_size = 100
            for p in position:
                if low + 1.01*size[p] < position[p][0] and position[p][0] < high - 1.01*size[p]:
                    count += 1
