...
class HW2Env(StackingEnv):
    ...
    def pick_up_object_with_internal_controller(self, seg_id: int, height: float) -> None:
        ...
        # move forward
        self.move_to_target_pose_with_internal_controller(ee2base, 100)
        self.wait_n_steps(50)
    ...
    def place_object_with_internal_controller(self, seg_id: int, target_object_position: np.ndarray) -> None:
        ...
        # # move up
        # gripper_pose[2,3] += 0.3
       
        # self.move_to_target_pose_with_internal_controller(gripper_pose, 100)
        # self.wait_n_steps(50)
   

    def user_controller(self, qvel: np.ndarray, target_ee_pose: np.ndarray) -> None:
        assert qvel.size == len(self.arm_joints)
        pids = []
        pid_parameters = [(40, 5, 2), (40, 5.0, 2), (30, 5.0, 1), (20, 5.0, 0.5), (5, 0.8, 0.4), (5, 0.8, 0.4),
                      (5, 0.8, 0.4), (0.1, 0, 0.02), (0.1, 0, 0.02)]

        target_qpos = qvel * self.scene.get_timestep() + self.robot.get_drive_target()[:-2]
        # print("qpos",target_qpos)
        
        for i, joint in enumerate(self.arm_joints):
            joint.set_drive_velocity_target(qvel[i])
            joint.set_drive_target(target_qpos[i])
        
        active_joints = [joint for joint in self.robot.get_joints() if joint.get_dof() > 0]
        for i, joint in enumerate(active_joints):
            pids.append(PID(pid_parameters[i][0], pid_parameters[i][1], pid_parameters[i][2]))
        
        target_pos = np.zeros(9)
        # target_pos[:-2] = self.compute_ik(target_ee_pose)[0]
        target_pos[:-2] = target_qpos
        target_pos[7:] = self.robot.get_drive_target()[-2: ]
        # print("pos",target_pos)
        

        compensation = self.robot.compute_passive_force(gravity=True, coriolis_and_centrifugal=True, external=False)
        # coriolis_and_centrifugal_compensation = self.robot.compute_passive_force(gravity=False, coriolis_and_centrifugal=True, external=False)
        pid_qf = pid_forward(pids, target_pos, self.robot.get_qpos(), self.scene.get_timestep())
        joint_torque = compensation + pid_qf
    ...
    def move_to_target_pose_with_user_controller(self, target_ee_pose: np.ndarray, num_steps: int) -> None:
        ...
        target_pos = self.compute_ik(target_ee_pose)[0]
       
        for i in range(num_steps):
        
            target_joint_velocity = self.compute_joint_velocity_from_twist(spatial_twist)

            self.user_controller(target_joint_velocity, target_ee_pose)
            self.step()
            self.render()   
            executed_time  -= self.scene.get_timestep()

            current_pos = self.robot.get_qpos()[:-2]    
            # if np.linalg.norm(current_pos - target_pos) < 1:
            #     break
    ...
    def pick_up_object_with_user_controller(self, seg_id: int, height: float) -> None:
        ...
        self.move_to_target_pose_with_user_controller(gripper_pose, 100)
        self.wait_n_steps(50)
