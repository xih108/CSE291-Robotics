...
from env.hw2_env import HW2Env
import numpy as np


def main():
    np.set_printoptions(precision=4)
    env = HW2Env(timestep=1 / 500)
    ...
    env.pick_up_object_with_internal_controller(blue_box.get_id(), pick_up_height)
    blue_height_after_pick = blue_box.get_pose().p[2]
    print("Target height of blue box: {}, achieved height of blue box: {}".format(pick_up_height + blue_height_before_pick,
                                                                              blue_height_after_pick))
    assert np.abs(pick_up_height + blue_height_before_pick - blue_height_after_pick) < 0.02, "Lift blue box task fail."
    env.place_object_with_internal_controller(blue_box.get_id(), blue_target_position)

    evaluate_final_results(env)


def evaluate_first_two_box(env: HW2Env):
...
def evaluate_final_results(env: HW2Env):
    if env.evaluate_final_result():
        print("Successfully stack all three boxes to target place")
        print("Press q to continue")
        while not env.should_quit():
            env.render()
    else:
        print("Fail to stack the three boxes to target place")
        print("Observe what happen and press q to quit")
        while not env.should_quit():
            env.render()
        env.scene = None
